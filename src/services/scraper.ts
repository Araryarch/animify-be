    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        elements.forEach((el) => {
          ...
          // Extract ID from URL, handle both /anime/slug and /slug formats
          const animeId = samehadakuUrl.split("/").filter(Boolean).pop() || "";

          // Title is in the "title" attribute or text content or h2/title class
          let title = linkEl?.getAttribute("title") || linkEl?.textContent?.trim() || "";
          if (!title) {
            title = el.querySelector(".title, h2")?.textContent?.trim() || "";
          }

          // Extract genres from article class names or content
          const genreList: any[] = [];
